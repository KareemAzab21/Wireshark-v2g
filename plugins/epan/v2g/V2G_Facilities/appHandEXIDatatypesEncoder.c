...
static int V2gFac_encode_appHandAppProtocolType(bitstream_t* stream,  appHandAppProtocolType* appHandAppProtocolType);
static int V2gFac_encode_appHandAnonType_supportedAppProtocolReq(bitstream_t* stream,  appHandAnonType_supportedAppProtocolReq* appHandAnonType_supportedAppProtocolReq);
static int V2gFac_encode_appHandAnonType_supportedAppProtocolRes(bitstream_t* stream,  appHandAnonType_supportedAppProtocolRes* appHandAnonType_supportedAppProtocolRes);
...
static int V2gFac_encode_appHandAnonType_supportedAppProtocolReq(bitstream_t* stream,  appHandAnonType_supportedAppProtocolReq* appHandAnonType_supportedAppProtocolReq) {
	...
	while(!done) {
		switch(grammarID) {
		case 7:
			/* FirstStartTag[START_ELEMENT(AppProtocol)] */ // SE for the first app protocol
			if (appHandAnonType_supportedAppProtocolReq_AppProtocol_currArrayIndex < appHandAnonType_supportedAppProtocolReq->AppProtocol.arrayLen ) {
				errn = encodeNBitUnsignedInteger(stream, 1, 0);
				if(errn == 0) {
					errn = V2gFac_encode_appHandAppProtocolType(stream, &appHandAnonType_supportedAppProtocolReq->AppProtocol.array [appHandAnonType_supportedAppProtocolReq_AppProtocol_currArrayIndex++]);
				}
				grammarID = 8;
			} else {
				errn = EXI_ERROR_UNKOWN_EVENT;
		...
		case 9:
			...
			if (appHandAnonType_supportedAppProtocolReq_AppProtocol_currArrayIndex < appHandAnonType_supportedAppProtocolReq->AppProtocol.arrayLen ) {
				...
				errn = encodeNBitUnsignedInteger(stream, 2, 1);  // end of Supported app protocol Request  0 1
				if(errn == 0) {
					done = 1;
				}
				grammarID = 6;
...
/************************************************************************************
* Service Name: V2gFac_encode_appHandExiDocument
* Service ID[hex]: 0x01
* Sync/Async: Synchronous
* Reentrancy: Non-Reentrant
* Parameters (in): Pointer to stream, Pointer to appHandEXIdocument
...
* Available via: appHnadEXIDatatybesEncoder.h
* Requirement:
************************************************************************************/
int V2gFac_encode_appHandExiDocument(bitstream_t* stream,  appHandEXIDocument* exiDoc) {
	...
	/* check if there is no errors in encoding EXI header step */
	if(errn == 0) {
		...
		/* check if the handshake type is request */
		if ( exiDoc->supportedAppProtocolReq_isUsed == 1u ) { 


			/* START_ELEMENT({urn:iso:15118:2:2010:AppProtocol}supportedAppProtocolReq) */
			/* encode 2 bites equal to 0 , unique for application handshake request */
			errn = encodeNBitUnsignedInteger(stream, 2, 0);

			/* check if there is no errors in encoding step */
			if(errn == 0) {

				/* call encoding function for handshake request */
				errn = V2gFac_encode_appHandAnonType_supportedAppProtocolReq(stream, &exiDoc->supportedAppProtocolReq );
			}

		/* if handshake type is response */
		} else if ( exiDoc->supportedAppProtocolRes_isUsed == 1u ) { 

			/* START_ELEMENT({urn:iso:15118:2:2010:AppProtocol}supportedAppProtocolRes) */
			/* encode 2 bites equal to 1, unique for application handshake response */
			errn = encodeNBitUnsignedInteger(stream, 2, 1);

			/* check if there is no errors in encoding step */
			if(errn == 0) {

				/* call encoding function for handshake response */
				errn = V2gFac_encode_appHandAnonType_supportedAppProtocolRes(stream, &exiDoc->supportedAppProtocolRes );
			}
		} else {

			/* not request, not response, report error due to unknown event */
			errn = EXI_ERROR_UNKOWN_EVENT;
		}
